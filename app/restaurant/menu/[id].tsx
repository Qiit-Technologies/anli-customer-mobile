    ...
    <View className="flex-1 bg-white">
      ...
      <View className="bg-[#3D2117] pt-14 pb-8 px-6 rounded-b-[40px] shadow-lg">
        ...
        <View className="items-center">
          <Text className="text-white text-3xl font-black text-center">{hotel?.name}</Text>
          <Text className="text-white/60 text-xs mt-1 uppercase tracking-[3px]">Culinary Showcase</Text>
        </View>
      </View>
      ...
      >
        {menus.length > 0 ? (
          menus.map((menu: any) => (
            <View key={menu.id} className="mt-8">
              ...
              <View className="px-6 mb-2">
                <Text className="text-3xl font-black text-[#FF8A00]">{menu.name}</Text>
                {menu.description && (
                  <Text className="text-[#8E9BAE] text-xs mt-1">{menu.description}</Text>
                )}
                <View className="w-20 h-1 bg-[#3D2117] mt-2 rounded-full" />
              </View>

              {menu.categories?.map((category: any, catIndex: number) => (
                <View key={category.id} className={catIndex === 0 ? "mt-4" : "mt-8"}>
                  {/* Category Header */}
                      ...
                      >
                        ...
                        <View className="ml-4 flex-1 justify-center">
                          <View className="flex-row justify-between items-start">
                            <Text className="text-base font-bold text-[#3D2117] flex-1 mr-2">{item.name}</Text>
                            <Text className="text-[#FF8A00] font-black text-base">₦{item.price}</Text>
                          </View>
                          <Text className="text-[#8E9BAE] text-[10px] leading-4 mt-1" numberOfLines={2}>
                            {item.description || 'Expertly prepared with the finest ingredients and traditional culinary techniques.'}
                          </Text>
                        </View>
                      </View>
                    ))}
          ...
          <View className="items-center justify-center py-24 px-10">
            ...
            <Text className="text-gray-400 text-center mt-4">Menu showcase coming soon!</Text>
          </View>
        )}
      </ScrollView>
    </View>
