  ...
  if (!hotel) {
    return (
      <View className="flex-1 bg-white items-center justify-center p-6">
        ...
        <TouchableOpacity
          onPress={() => router.back()}
          className="bg-[#007AFF] px-8 py-3 rounded-xl"
        >
        ...
        <View className="px-6 mb-8">
          ...
          <View className="flex-row items-center mb-4">
            ...
            <View className="flex-row items-center ml-4">
              ...
              <Text className="text-[#8E9BAE] text-xs ml-1">
                {hotel.tags || "General Restaurant"}
              </Text>
            </View>
          </View>
          ...
          >
            <View className="flex-row border-b border-[#F1F5F9] pb-0.5">
              {TABS.map((tab) => (
                ...
                >
                  <Text
                    className={`text-base font-medium ${activeTab === tab ? "text-[#FF8A00]" : "text-[#8E9BAE]"}`}
                  >
                    {tab}
                  </Text>
                </TouchableOpacity>
        ...
        <View className="px-6 pb-20">
          {activeTab === "Available Bookings" && (
            <View>
                ...
                {TIME_SLOTS.map((slot, index) => (
                  ...
                  >
                    <Ionicons
                      ...
                      color="#1A202C"
                    />
                    <Text className="text-sm font-medium text-[#1A202C] ml-2">
                      {slot}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <View className="mb-8">
                <Image
                  source={{
                    uri:
                      hotel.coverImage ||
                      "https://images.unsplash.com/photo-1550966842-28c2e2ad4455?auto=format&fit=crop&w=600&q=80",
                  }}
              ...
              <View className="items-center">
                ...
                >
                  <Text className="text-[#3D2117] text-center font-bold">
                    Set other Bookings Option
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity>
                  <Text className="text-[#3D2117] text-sm text-center">
                    Notify Me for available space
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View>
              ...
              {menuItems.slice(0, 3).length > 0 ? (
                menuItems.slice(0, 3).map((item: any) => (
                  <View
                    key={item.id}
                    className="flex-row items-center bg-[#F8FAFC] p-4 rounded-[24px] mb-4"
                  >
                    <Image
                      source={{
                        uri:
                    ...
                    <View className="ml-4 flex-1">
                      ...
                      <Text
                        className="text-[#8E9BAE] text-xs mt-1"
                        numberOfLines={2}
                      >
                        {item.description}
                      </Text>
                      <Text className="text-[#FF8A00] font-bold mt-2">
                        ₦{item.price}
                      </Text>
                    </View>
                  </View>
          ...
          {activeTab === "Details" && (
            <View>
              ...
              <View className="mb-8">
                <Text className="text-base font-bold text-[#3D2117] mb-2">
                  About
                </Text>
                <Text className="text-[#8E9BAE] text-sm leading-6">
                  Welcome to {hotel.name}! We're your go-to place for fine
                  dining and bespoke experiences.
                </Text>
              </View>
              ...
              <View className="h-48 rounded-[32px] overflow-hidden bg-[#F8FAFC]">
                <Image
                  source={{
                    uri: "https://images.unsplash.com/photo-1526772662000-3f88f10405ff?auto=format&fit=crop&w=600&q=80",
                  }}
                  style={{ width: "100%", height: "100%" }}
                  contentFit="cover"
                />
                <View className="absolute inset-0 bg-black/10 items-center justify-center p-6">
                  <View className="bg-white px-4 py-2 rounded-full flex-row items-center shadow-sm">
                    ...
                    <Text
                      className="text-[10px] ml-1 font-medium text-gray-800"
                      numberOfLines={1}
                    >
                      {hotel.address}
                    </Text>
